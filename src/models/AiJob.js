...
const schema=new Schema({
 ...
 attempts:{type:Number,default:0,min:0,max:10},availableAt:{type:Date,default:Date.now,index:true},startedAt:Date,lockedUntil:{type:Date,default:null},completedAt:Date,expiresAt:{type:Date,index:true},errorCode:{type:String,default:'',maxlength:100},errorMessage:{type:String,default:'',maxlength:1000},
...
schema.index({status:1,availableAt:1,createdAt:1});schema.index({status:1,lockedUntil:1,availableAt:1,createdAt:1});schema.index({actorUserId:1,createdAt:-1});schema.index({storeId:1,createdAt:-1});
